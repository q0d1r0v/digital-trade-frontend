...
export const AdminUsersPage = () => {
  ...
  const loadUsers = async () => {
    try {
      Loading.show();
      const edpoint = API_ENDPOINTS.ADMIN_USER.LOAD_USERS;
  ...
  const createUser = async () => {
    try {
      ...
      if (createFormData.userID) {
        const edpoint = API_ENDPOINTS.ADMIN_USER.UPDATE_USER(
        ...
        const edpoint = API_ENDPOINTS.ADMIN_USER.CREATE_USER;
  ...
  const showEditUserDialog = async (user: UserDataTypes) => {
    try {
      Loading.show();
      await loadRoles();
      const edpoint = API_ENDPOINTS.ADMIN_USER.LOAD_USER(user.id);
      const { data } = await api.get(edpoint, {
        params: {
          includes: "profile",
        },
  ...
  const deleteUser = async () => {
    try {
      ...
      const endpoint = API_ENDPOINTS.ADMIN_USER.DELETE_USER(
        createFormData.userID,
      );
